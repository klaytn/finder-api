...
@EnableJpaRepositories(
    basePackages = [DbConstants.baseDomainPackage, DbConstants.set4DomainPackage],
    entityManagerFactoryRef = DbConstants.set4EntityManagerFactory,
    transactionManagerRef = DbConstants.set4TransactionManager
)
@EnableConfigurationProperties(FinderDbProperties::class)
class FinderDbSet4Config {
    @Bean
    fun set4DataSource(finderDbProperties: FinderDbProperties): HikariDataSource =
        HikariDataSource(finderDbProperties.set04.dataSource)

    @Bean
    fun set4EntityManagerFactory(
        finderDbProperties: FinderDbProperties,
        set4DataSource: DataSource,
    ): LocalContainerEntityManagerFactoryBean {
        val enableJpaRepositories = this.javaClass.getAnnotation(EnableJpaRepositories::class.java)

        val dataSource = LazyConnectionDataSourceProxy(set4DataSource)
        val vendorAdapter = HibernateJpaVendorAdapter()

        val dbProperty = finderDbProperties.set04
        ...
        localContainerEntityManagerFactoryBean.persistenceUnitName = DbConstants.set4EntityManager
    ...
    @Bean
    fun set4TransactionManager(set4EntityManagerFactory: EntityManagerFactory): PlatformTransactionManager {
        val transactionManager = JpaTransactionManager()
        transactionManager.entityManagerFactory = set4EntityManagerFactory
