...
@Repository
interface TokenTransferRepository : BaseRepository<TokenTransfer> {
        ...
        value = """
            (
                (
                    ...
                    FROM 
                        ${DbTableConstants.tokenTransfers} force index(ix_from_contractaddress_blocknumber_displayorder) 
                    WHERE 
                ...
                (
                    SELECT 
                        id, block_number, display_order 
                    FROM 
                        ${DbTableConstants.tokenTransfers} force index(ix_to_contractaddress_blocknumber_displayorder) 
                    WHERE 
            ...
            (
                (
                    SELECT 
                        id, block_number, display_order 
                    FROM 
                        ${DbTableConstants.tokenTransfers} force index(ix_from_blocknumber_displayorder) 
                    WHERE 
                        `from` = :accountAddress 
                    ORDER BY 
                        block_number DESC, display_order DESC 
                ...
                (
                    SELECT 
                        id, block_number, display_order 
                    FROM 
                        ${DbTableConstants.tokenTransfers} force index(ix_to_blocknumber_displayorder) 
                    WHERE 
                        `to`   = :accountAddress 
                    ORDER BY 
                        block_number DESC, display_order DESC 
    ...
    @Query(
        ...
        value = """
            SELECT 
                id 
            FROM 
                ${DbTableConstants.tokenTransfers} 
            WHERE 
                contract_address = :contractAddress 
            ORDER BY 
                block_number DESC, display_order DESC
    ...
    @Query(
        ...
        value = """
            SELECT
                CASE WHEN EXISTS (
                (
                    (select id from ${DbTableConstants.tokenTransfers} where `from` = :accountAddress limit 1)
                    union
                    (select id from ${DbTableConstants.tokenTransfers} where `to` = :accountAddress limit 1)
                )
            )
            THEN 'true'
            ELSE 'false'
