...
(
    ...
    KEY `ix_accountaddress_contractaddress_blocknumber`
        (`account_address`, `contract_address`, `block_number` DESC),
    KEY `ix_accountaddress_spenderaddress_blocknumber`
        (`account_address`, `spender_address`, `block_number` DESC),
    KEY `ix_accountaddress_blocknumber`
        (`account_address`, `block_number` DESC),
    KEY `ix_spenderaddress_blocknumber`
        (`spender_address`, `block_number` DESC)
) ENGINE = InnoDB
  DEFAULT CHARSET = utf8mb4
  COLLATE = utf8mb4_unicode_ci;
...
(
    ...
    `updated_at`        datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6) COMMENT 'modified date',
    PRIMARY KEY (`id`),
    KEY `ix_accountaddress_contracttype_approvedall_blocknumber`
        (`account_address`, `contract_type`, `approved_all`, `block_number` DESC),
    KEY `ix_accountaddress_contractaddress_approvedall_blocknumber`
        (`account_address`, `contract_address`, `approved_all`, `block_number` DESC),
    KEY `ix_accountaddress_spenderaddress_approvedall_blocknumber`
        (`account_address`, `spender_address`, `approved_all`, `block_number` DESC),
    KEY `ix_accountaddress_approvedall_blocknumber`
        (`account_address`, `approved_all`, `block_number` DESC),
    KEY `ix_spenderaddress_approvedall_blocknumber`
        (`spender_address`, `approved_all`, `block_number` DESC)
) ENGINE = InnoDB
  DEFAULT CHARSET = utf8mb4
  COLLATE = utf8mb4_unicode_ci;
