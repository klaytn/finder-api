...
sudo docker build --build-arg VERSION="$VERSION" -f ./deploy/front-api/dockerfile -t $(aws sts get-caller-identity --query Account --output text).dkr.ecr.ap-northeast-2.amazonaws.com/finder-api:"$VERSION" .
