...
class EventLogToListViewMapper(
    ...
    override fun transform(source: List<EventLog>): List<EventLogListView> {
        return source.map {
            getEventLogListView(it)
        }
    }

    private fun getEventLogListView(source: EventLog): EventLogListView {
        val topics = source.topics
        if(topics.isEmpty()) {
            return unknownEvent(source)
        }
    ...
    private fun unknownEvent(source: EventLog) = EventLogListView(
        logIndex = source.logIndex,
        contractAddress = source.address,
        type = "Unknown",
        topics = source.topics,
